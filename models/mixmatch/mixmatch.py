...
class MixMatch:
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (_, x_ulb_w1, x_ulb_w2) in zip(
            ...
            x_lb, x_ulb_w1, x_ulb_w2 = (
                x_lb.cuda(args.gpu),
                x_ulb_w1.cuda(args.gpu),
                x_ulb_w2.cuda(args.gpu),
            )
            ...
            with amp_cm():
                with torch.no_grad():
                    ...
                    # avg
                    avg_prob_x_ulb = (
                        torch.softmax(logits_x_ulb_w1, dim=1)
                        + torch.softmax(logits_x_ulb_w2, dim=1)
                    ) / 2
                    avg_prob_x_ulb = avg_prob_x_ulb / avg_prob_x_ulb.sum(
                        dim=-1, keepdim=True
                    )
                    # sharpening
                    ...
                    sharpen_prob_x_ulb = (
                        sharpen_prob_x_ulb
                        / sharpen_prob_x_ulb.sum(dim=-1, keepdim=True)
                    ).detach()

                    # Pseudo Label
                    input_labels = torch.cat(
                        [
                            one_hot(y_lb, args.num_classes, args.gpu),
                            sharpen_prob_x_ulb,
                            sharpen_prob_x_ulb,
                        ],
                        dim=0,
                    ...
                    mixed_x, mixed_y, _ = mixup_one_target(
                        inputs, input_labels, args.gpu, args.alpha, is_bias=True
                    )
                ...
                rampup = float(
                    np.clip(self.it / (args.ramp_up * args.num_train_iter), 0.0, 1.0)
                )
