...
class ReMixMatch:
    ...
    def train(self, args, logger=None):
        ...
        dist_file_name = (
            r"./data_statistics/" + args.dataset + "_" + str(args.num_labels) + ".json"
        )
        ...
        for (_, x_lb, y_lb), (
            ...
            x_ulb_s1_rot,
            rot_v,
        ) in zip(self.loader_dict["train_lb"], self.loader_dict["train_ulb"]):
            ...
            with amp_cm():
                with torch.no_grad():
                    ...
                    if p_model == None:
                        ...
                        p_model = (
                            p_model * 0.999
                            + torch.mean(prob_x_ulb.detach(), dim=0) * 0.001
                        )
                    ...
                    mixed_x, mixed_y, _ = mixup_one_target(
                        mixed_inputs, input_labels, args.gpu, args.alpha, is_bias=True
                    )
                ...
                # ramp for w_match
                w_match = args.w_match * float(
                    np.clip(self.it / (args.warm_up * args.num_train_iter), 0.0, 1.0)
                )
                w_kl = args.w_kl * float(
                    np.clip(self.it / (args.warm_up * args.num_train_iter), 0.0, 1.0)
                )
