    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(
            ...
            with amp_cm():
                ...
                tsa = TSA(
                    args.TSA_schedule, self.it, args.num_train_iter, args.num_classes
                )  # Training Signal Annealing
                sup_mask = (
                    torch.max(torch.softmax(logits_x_lb, dim=-1), dim=-1)[0]
                    .le(tsa)
                    .float()
                    .detach()
                )
                ...
                sup_loss = (
                    ce_loss(logits_x_lb, y_lb, reduction="none") * sup_mask
                ).mean()
